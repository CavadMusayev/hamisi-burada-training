...
#pragma warning disable //ALL
        ...
        */
        ...
        [BeforeSpec]
        public async void BeforeSpec()
        {
            if (CONSTANTlar.ENABLE_OBS)
            {
                OBS_Application.LaunchOBS();
                Thread.Sleep(5000);
                await OBSSocket.OpenConnection();
                await OBSSocket.StartRecording();
            }
        ...
        [AfterSpec]
        public async void AfterSpec()
        {
            if (CONSTANTlar.ENABLE_OBS)
            {
                await OBSSocket.StopRecording();
                await OBSSocket.CloseConnection();
            }
...
[Step("Go Into View: xpath = <xpath>")]
public void GoIntoView(string xpath)
{
    ...
    while (!element.Displayed && currentScroll < maxScrolls)
    {
...
//         }
        ...
        public void Əgər_siyahi_xana_boşsa_digər_xananın_dəyərini_dəyiş(string siyahi_XPathi, string digər_siyahi_xana_XPathi)
        {
            ...
            do
            {
                ...
                if (məlumat_yoxdur_var_mi_yox_mu.Count > 0)
                { // yani siyahi xana boşdu
        ...
        [Step("Məhsullar qiymətə görə azalan sırada sıralanmalıdır: XPath=<xpath>")]
        public void MehsullarQiymeteGoreAzalanSiradaSiralanmalidir(string xpath)
        {
            IReadOnlyCollection<IWebElement> priceElements = driver.FindElements(By.XPath(xpath));

            List<double> prices = new List<double>();

            foreach (IWebElement el in priceElements)
            {
                // string priceText = el.Text.Replace("₼", "").Trim().Replace(",", ".");
                string priceText = el.Text.Replace("₼", "").Replace(".", "").Replace(",", ".").Trim();
                ...
                {
                    Console.WriteLine($"Qiymət parse edilə bilmədi: {priceText}");
                }
            }

            // Azalan sıraya görə sıralanmış halı
            List<double> sortedPrices = prices.OrderByDescending(x => x).ToList();

            for (int i = 0; i < sortedPrices.Count; i++)
            {
                var a = sortedPrices[i];
                var b = sortedPrices[++i];
                if (a > b) {
                       System.Console.WriteLine("TEST KECDI");
                       Thread.Sleep(1000);
                } else
                    Assert.Fail();
            }
            ...
            // Assert ilə müqayisə
            // Assert.IsTrue(prices.SequenceEqual(sortedPrices), "❌ Məhsullar qiymətə görə azalan sırada sıralanmayıb.");
        ...
        public void Dəyişgən_və_dəyərini_yadda_saxla(string Dəyişgən, string dəyər)
        {
            ...
            //         driver.Navigate().GoToUrl("https://www.hamisiburada.az/..."); // Buraya test etdiyin URL-i yaz

            //         // Sayfanın tam yüklənməsi üçün kiçik gözləmə (istəyə görə WebDriverWait istifadə et)
            //         Thread.Sleep(2000);

            //         // Qiymət elementlərini topla
            //         IReadOnlyCollection<IWebElement> priceElements = driver.FindElements(By.XPath("//span[@class='fw-700 text-primary']"));

            //         List<double> prices = new List<double>();

            //         foreach (IWebElement el in priceElements)
            //         {
            //             string priceText = el.Text.Replace("₼", "").Trim().Replace(",", "."); // ₼ sil, vergülü nöqtəyə çevir
            ...
            //         if (prices.SequenceEqual(sortedPrices))
            //         {
            //             Console.WriteLine("✅ Qiymətlər düzgün şəkildə azalan sıradadır.");
            //         }
            //         else
            //         {
            //             Console.WriteLine("❌ Qiymətlər düzgün azalan sırada deyil.");
            //             Console.WriteLine("Tapılan qiymətlər:");
        ...
        // [Step("Dəyişgən və dəyərini yadda saxla: <Deyisgen> = <deyer>")]
        // public void Deyeri_Yadda_Saxla(string Deyisgen)
        // {
        ...
        */
        ...
        public void YOXLA_sehife_URLi_bumu(String beklenen_URL)
        {
            ...
            if (current_url != beklenen_URL)
            {
                TestLogaYaz($"{beklenen_URL} budur, lakin {current_url}-dədir");
                Assert.Fail();
            }
        }
        ...
        {
            if (Əsnək_String_Dəyişgənlər_Cədvəli[dəyişgən1].CompareTo(Əsnək_String_Dəyişgənlər_Cədvəli[dəyişgən2]) == 0)
            { // beraber dirler 
...
*/
        ...
        */
        ...
        public void YOXLA_Səhifədə_heçbir_xana_xata_qırmızı_ikonun_olub_olmadığını()
        {//(string optional_xata_varsa_əmr) {
        ...
        #region Elnur's Steps
        [Step("Dinamik scroll et <dərəcə>")]
        public void Dinamik_scroll_et(string degree)
        {
            ((IJavaScriptExecutor)driver).ExecuteScript($"window.scrollTo(0, {degree})");
            TestLogaYaz($"Dinamik scroll dərəcəsi: {degree}");
        }

        [Step("Səhifəyə get: <url>")]
        ...
        [Step("Kursoru XPATH ilə hərəkət etdir <XPATH>")]
        public void MoveCursorWithXPath(string XPATH)
        {
            TestLogaYaz($"Kursoru hərəkət etdirirəm (XPath ilə): {XPATH}");
            try
            {
                var element = new WebDriverWait(driver, TimeSpan.FromSeconds(CONSTANTlar.SELENIUM_ELEMENT_GOZLEME_SANIYESI)).Until(
                   ExpectedConditions.ElementToBeClickable(
                       By.XPath(XPATH)));
        ...
        [Step("Kursoru ID ilə hərəkət etdir <ID>")]
        public void MoveCursorWithId(string Id)
        {
            TestLogaYaz($"Kursoru hərəkət etdirirəm (İD ilə): {Id}");
            try
            {
                var element = new WebDriverWait(driver, TimeSpan.FromSeconds(CONSTANTlar.SELENIUM_ELEMENT_GOZLEME_SANIYESI)).Until(
                    ExpectedConditions.ElementToBeClickable(
        ...
        [Step("Kursoru mətn-ə hərəkət etdir <Text>")]
        public void MoveCursorWithText(string text)
        {
            TestLogaYaz($"Kursoru hərəkət etdirirəm (mətn ilə): {text}");
            try
            {
                var element = new WebDriverWait(driver, TimeSpan.FromSeconds(CONSTANTlar.SELENIUM_ELEMENT_GOZLEME_SANIYESI * 3)).Until(
                    ExpectedConditions.ElementToBeClickable(
        ...
        [Step("Kursoru class-ə hərəkət etdir <Classname>")]
        public void MoveCursorWithClassname(string classname)
        {
            TestLogaYaz($"Kursoru hərəkət etdirirəm (classname ilə): {classname}");
            try
            {
                var element = new WebDriverWait(driver, TimeSpan.FromSeconds(CONSTANTlar.SELENIUM_ELEMENT_GOZLEME_SANIYESI * 3)).Until(
                    ExpectedConditions.ElementToBeClickable(
        ...
        [Step("Video qeydi başlat")]
        public async void StartVideoRecord()
        {
            OBSSocket.OpenConnection();
            OBSSocket.StartRecording();
        }

        [Step("Video qeydi dayandır")]
        public async void StopVideoRecord()
        {
            OBSSocket.StopRecording();
            OBSSocket.CloseConnection();
        }
        ...
        [Step("Ekranı kiçik zoom et <yuzde> %")]
        public void Ekranı_Kicik_Zoom_Et(string yuzde = "75")  // Default olaraq 75% təyin edirik
        {
            TestLogaYaz("Ekranı_Kicik_Zoom_Et: " + yuzde + "%");

            IJavaScriptExecutor jsExecutor = (IJavaScriptExecutor)driver;
            jsExecutor.ExecuteScript("document.body.style.zoom='" + yuzde + "%'");

            // Bu kod göstərilən 'yuzde' ilə ekranın ölçüsünü dəyişir.
        }
